...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const success = await login(username, password); // Using plain password for demo
      if (!success) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-brand-dark">
      <div className="bg-brand-card p-8 rounded-lg border border-brand-border shadow-lg w-full max-w-sm">
        ...
        <p className="text-center text-brand-light text-sm mb-4">Log in with a user from the database. <br /> e.g., admin / adminpass</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="username" className="block text-sm font-medium text-brand-light">Username</label>
            <input
              type="text"
              id="username"
              value={username}
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-brand-light">Password</label>
            <input
              type="password"
              id="password"
              value={password}
