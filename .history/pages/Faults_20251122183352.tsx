...
const Faults: React.FC = () => {
    ...
    const filteredFaults = faults?.filter(fault => {
        ...
        return (
            ...
            format(new Date(fault.reportDate), 'dd.MM.yyyy HH:mm').toLowerCase().includes(searchLower)
        );
    });
                                ...
                                filteredFaults.map(fault => (
                                    <tr key={fault.id} className="border-b border-brand-border hover:bg-brand-dark">
                                        <td className="p-3 whitespace-nowrap">
                                            <div className="text-white">{format(new Date(fault.reportDate), 'dd.MM.yyyy HH:mm')}</div>
