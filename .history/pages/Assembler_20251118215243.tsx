...
const Assembler: React.FC = () => {
    ...
    const handleSwap = async (e: React.FormEvent) => {
        ...
        try {
            if (swapState.swapType === 'Assembly') {
                ...
                if (!swapState.removeId || !swapState.installId) {
                ...
                // Optional: Show warning if part numbers don't match (but allow the swap)
                if (inventoryItem.partNumber !== removedComponent.partNumber) {
                    console.log('Warning: Different part numbers', {
                        removed: removedComponent.partNumber,
                        installed: inventoryItem.partNumber
                    });
                    // Allow swap to continue - no restriction
                }
                ...
                // Hours and life limit are preserved from inventory item itself
                ...
                await inventoryApi.create({
                    ...
                    currentHours: removedComponent.currentHours, // Save working hours
                    lifeLimit: removedComponent.lifeLimit // Save life limit
                });
    ...
    const getComponentDescription = (id: number | null, serialNumber?: string) => {
        ...
        // Try to find in engine components
        if (engines) {
                ...
                <div className="bg-brand-card p-6 rounded-lg border border-brand-border">
                    ...
                    <form onSubmit={handleSwap} className="space-y-4">
                        ...
                        {swapState.swapType === 'Assembly' ? (
                            ...
                            swapState.engineId && (
                                <>
                                    <div>
                                        ...
                                        >
                                            ...
                                            {engineComponents.length > 0 ? (
                                                ...
                                                <option value="" disabled>Motor üzerinde parça bulunamadı</option>
                                            )}
                                        </select>
                                        {engineComponents.length === 0 && (
                                            <p className="text-sm text-yellow-400 mt-1">
                                    ...
                                    {/* Info box for component swap */}
                                    {swapState.removeId && swapState.installId && selectedRemoveComponent && (
                                        <div className="bg-green-500/10 border border-green-500/30 rounded-md p-3 text-sm">
                                            <p className="font-semibold text-green-400 mb-2">✅ Parça Değişimi Hazır:</p>
                                            <ul className="list-disc list-inside text-brand-light ml-2 space-y-1">
                                                ...
                                                <li>Takılan parça depodan çıkarılacak</li>
                                                <li>Yeni parçanın saati: <strong>{selectedEngine?.totalHours}h</strong> (motor saati ile senkron)</li>
                                            </ul>
                                        </div>
                                    )}
                                </>
                        ...
                        <button 
                            ...
                            disabled={
                                !swapState.engineId ||
                                (swapState.swapType === 'Component' && (!swapState.removeId || !swapState.installId)) ||
                        ...
                        <tbody>
                            {swaps?.map(swap => (
                                <tr key={swap.id} className="border-b border-brand-border hover:bg-brand-dark transition-colors">
                                    <td className="p-3">{new Date(swap.swapDate).toLocaleDateString('tr-TR')}</td>
                                    ...
                                    {canModify && (
                                        <td className="p-3 flex space-x-4">
                                            <button disabled className="text-gray-600 cursor-not-allowed" title="Düzenleme devre dışı">
                                                <PencilIcon />
                                            </button>
                                            <button 
