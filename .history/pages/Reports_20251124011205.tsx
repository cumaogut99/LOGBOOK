  ...
  const tests = useQuery(() => testsApi.getAll(), []);
    ...
    const reportData = useMemo(() => {
        ...
        if (activityTypes.tests) {
            if (reportScope === 'assembly') {
                // Alt montaj grubu modunda: assembly'nin bulunduğu motordaki testleri göster
                if (selectedAssemblyGroup) {
                    // Assembly'nin hangi motorda olduğunu bul
                    const assemblyLocation = allAssemblyGroups.find(a => a.serialNumber === selectedAssemblyGroup);
                    
                    if (assemblyLocation && assemblyLocation.location !== 'Depo') {
                        // Motor üzerinde, o motordaki testleri göster
                        const engineId = engines.find(e => e.serialNumber === assemblyLocation.location)?.id;
                        
                        tests.forEach(test => {
                            if (isDateValid(test.testDate) && test.engineId === engineId) {
        ...
        if (activityTypes.maintenance) {
            if (reportScope === 'assembly') {
                // Alt montaj grubu modunda: assembly'nin bulunduğu motordaki bakımları göster
                if (selectedAssemblyGroup) {
                    const assemblyLocation = allAssemblyGroups.find(a => a.serialNumber === selectedAssemblyGroup);
                    ...
                    if (assemblyLocation && assemblyLocation.location !== 'Depo') {
                        ...
                        maintenanceHistory.forEach(m => {
                            if (isDateValid(m.performedDate) && m.engineId === engineId) {
    ...
    }, [engines, faults, tests, swaps, maintenanceHistory, inventory, dateRange, reportScope, selectedEngineId, selectedAssemblyGroup, selectedComponentSerial, activityTypes, subFilters, componentEngineHistory, allAssemblyGroups]);
