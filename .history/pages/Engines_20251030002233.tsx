...
const Engines: React.FC = () => {
    ...
    if (!engines) return <div>Loading engines...</div>;
    ...
    if (selectedEngine) {
        return <EngineDetails engine={selectedEngine} onBack={handleBack} />;
    }

    return <EngineFleetOverview engines={engines} onSelectEngine={handleSelectEngine} />;
};

const EngineFleetOverview: React.FC<{ engines: Engine[], onSelectEngine: (engine: Engine) => void }> = ({ engines, onSelectEngine }) => {
    return (
        <div>
            <div className="flex justify-end mb-6">
                <button className="bg-brand-primary hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-md flex items-center">
...
const EngineDetails: React.FC<{ engine: Engine, onBack: () => void }> = ({ engine, onBack }) => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-brand-card p-4 rounded-lg border border-brand-border space-y-3">
                    ...
                    <div className="flex items-center justify-between"><span className="font-semibold">Status:</span>
                        <select defaultValue={engine.status} className="bg-brand-dark border border-brand-border rounded-md p-1 text-white text-sm">
