    ...
    const filteredMaintenancePlans = useMemo(() => {
        ...
        if (maintenanceSubTab === 'one-time') {
            ...
            if (periodicTab === 'pending') {
                // Show only those that are due (both parent periodic and auto-generated)
                filtered = duePlans?.filter(p => 
                    p.maintenanceType === 'periodic' || 
                    (p.maintenanceType === 'one-time' && p.description?.includes('Otomatik Oluşturuldu'))
                ) || [];
            } else if (periodicTab === 'completed') {
                // Show completed periodic maintenances (including auto-generated ones)
                filtered = filtered.filter(p => 
                    p.status === 'Completed' && 
                    (p.maintenanceType === 'periodic' || p.description?.includes('Otomatik Oluşturuldu'))
                );
            } else {
                // Planned - show:
                // 1. Active/Approved periodic plans (parent plans)
                // 2. Pending one-time plans that were auto-generated from periodic plans
                filtered = maintenancePlans.filter(p => {
                    // Show parent periodic plans
                    if (p.maintenanceType === 'periodic' && (p.status === 'Active' || p.status === 'Approved')) {
                        return true;
                    }
                    // Show auto-generated one-time plans that are pending
                    if (p.maintenanceType === 'one-time' && 
                        p.description?.includes('Otomatik Oluşturuldu') && 
                        p.status === 'Pending') {
                        return true;
                    }
                    return false;
