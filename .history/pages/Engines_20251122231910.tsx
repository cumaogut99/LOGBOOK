...
const EngineDetails: React.FC<{ 
    ...
    const handleBRUpload = async () => {
        ...
        try {
            ...
            // Check for duplicates in the entire system
            const serialsToCheck = new Set<string>();
            rows.forEach((row: any) => {
                const sn1 = row.altAssySerialNumber?.toString().trim();
                const sn2 = row.parcaSeriNumarasi?.toString().trim();
                
                if (sn1 && sn1 !== '-' && sn1 !== 'N/A') serialsToCheck.add(sn1);
                if (sn2 && sn2 !== '-' && sn2 !== 'N/A') serialsToCheck.add(sn2);
            });

            if (serialsToCheck.size > 0) {
                const result = await enginesApi.checkDuplicates(Array.from(serialsToCheck), engine.id);
                if (result.duplicates && result.duplicates.length > 0) {
                    const errorMsg = "Yükleme iptal edildi! Aşağıdaki parçalar zaten sistemde (başka bir motorda veya depoda) kayıtlı:\n\n" + 
                        result.duplicates.slice(0, 10).map((d: any) => `• ${d.serialNumber} -> ${d.location}`).join('\n') +
                        (result.duplicates.length > 10 ? `\n...ve ${result.duplicates.length - 10} parça daha.` : '');
                    showError(errorMsg);
