...
const Tests: React.FC = () => {
    ...
    const handleLogSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let assemblyInfo: { name: string; partNumber: string; serialNumber: string } | null = null;
            
            if (isEngineTest) {
                actualEngineId = parseInt(newTest.engineId.replace('engine-', ''));
            } else if (isAssemblyTest) {
                // Alt montaj grubu için engine ID = 0 (motor dışı test)
                actualEngineId = 0;
                
                // Alt montaj grubu bilgilerini bul
            ...
            const createdTest = await testsApi.create({
                ...
                // Alt montaj grubu bilgileri (varsa)
                assemblyGroupName: assemblyInfo?.name,
                assemblyPartNumber: assemblyInfo?.partNumber,
                assemblySerialNumber: assemblyInfo?.serialNumber,
            });
                            ...
                            <form onSubmit={handleLogSubmit} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    >
                                        ...
                                        <optgroup label="📦 Alt Montaj Grupları (Depoda)">
                                    ...
                                    {filteredTests && filteredTests.length > 0 ? (
                                        filteredTests.map(test => (
                                            <tr key={test.id} className="border-b border-brand-border hover:bg-brand-dark">
                                                ...
                                                <td className="p-3">
                                                    {test.engineId === 0 && test.assemblyGroupName
                                                        ? <span className="text-purple-400">📦 {test.assemblyGroupName}<br/><span className="text-xs text-brand-light">PN: {test.assemblyPartNumber} | SN: {test.assemblySerialNumber}</span></span>
                                                        : test.engineId === 0 
