...
// Interface for assembly group info
interface AssemblyGroupInfo {
    name: string;
    partNumber: string;
    serialNumber: string;
}
...
function extractAssemblyGroups(components: Component[]): AssemblyGroupInfo[] {
    const groupsMap = new Map<string, AssemblyGroupInfo>();
    
    function traverse(comps: Component[]) {
        for (const comp of comps) {
            // Top-level components with children are assembly groups
...
const Assembler: React.FC = () => {
    ...
    const engineComponents = useMemo(() => {
        ...
        const allComponents = flattenComponents(selectedEngine.components);
        // Filter out assembly groups (only show individual parts in Component mode)
        return allComponents.filter(comp => !comp.children || comp.children.length === 0);
    }, [selectedEngine]);
    ...
    const filteredEngineComponents = useMemo(() => {
        ...
        return engineComponents.filter(comp => 
            ...
            (comp.assemblyGroup && comp.assemblyGroup.toLowerCase().includes(query))
        );
    ...
    const handleSwap = async (e: React.FormEvent) => {
        ...
        try {
            if (swapState.swapType === 'Assembly') {
                ...
                if (!swapState.removeId) {
                    showError('⚠️ MOTORDAN ÇIKARILACAK parçayı seçmediniz! Lütfen listeden bir parça seçin.');
                    return;
                }
                
                if (!swapState.installId && !swapState.installLater) {
                    showError('⚠️ DEPODAN TAKILACAK parçayı seçmediniz! Listeden parça seçin veya "sonra takılacak" işaretleyin.');
                    return;
                }
    ...
    // Helper to get detailed component info for history table
    const getDetailedComponentInfo = (id: number | null, serialNumber?: string, isAssembly: boolean = false) => {
        ...
        if (!id && serialNumber) {
            ...
            if (engines) {
                for (const engine of engines) {
                    const allComps = flattenComponents(engine.components);
    ...
    const getComponentDescription = (id: number | null, serialNumber?: string) => {
        ...
        if (!id) {
            // No ID and no serial number → truly nothing
            return 'N/A';
        }
    ...
    const handleUpdateSwap = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!editingSwap || !editingSwap.id || !user) return;

        try {
            // Check if user is installing a new part (for "install later" swaps)
            const newPartId = (editingSwap as any).newPartId;
            
            if (newPartId && engines) {
                const inventoryItem = inventory?.find(i => i.id === parseInt(newPartId));
                const engine = engines.find(e => e.id === editingSwap.engineId);
                
                if (!inventoryItem || !engine) {
                    showError('Parça veya motor bulunamadı');
                    return;
                }
                ...
                // Add to engine
                const updatedComponents = [...engine.components, newComponent];
                ...
                await inventoryApi.delete(parseInt(newPartId));
                
                // Update swap record
                await swapsApi.update(editingSwap.id, {
                    swapDate: editingSwap.swapDate,
                    componentInstalledId: parseInt(newPartId),
                    installedSerialNumber: inventoryItem.serialNumber
                });
                
                showSuccess('Parça başarıyla takıldı ve kayıt güncellendi!');
            } else {
                // Just update date
                await swapsApi.update(editingSwap.id, {
                    swapDate: editingSwap.swapDate
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'swap' && (
                <div className="bg-brand-card p-6 rounded-lg border border-brand-border">
                    {canSwap ? (
                        <>
                            <h2 className="text-lg font-bold text-white mb-4">Yeni Değişim İşlemi</h2>
                                        ...
                                        >
                                            ...
                                            {availableAssemblyGroups.length > 0 ? (
                                                availableAssemblyGroups.map(group => (
                                                    <option key={`${group.partNumber}-${group.serialNumber}`} value={group.name}>
                                                        {group.name} (PN: {group.partNumber}, SN: {group.serialNumber})
                                                    </option>
                                                ))
                                            ) : (
                                        ...
                                        >
                                            ...
                                            {inventoryAssemblyGroups.length > 0 ? (
                                                inventoryAssemblyGroups.map(group => {
                                                    const count = inventory?.filter(i => i.assemblyGroup === group.name).length || 0;
                                                    return (
                                                        <option key={`${group.partNumber}-${group.serialNumber}`} value={group.name}>
                                                            {group.name} (PN: {group.partNumber}, SN: {group.serialNumber}) - {count} parça
                                                        </option>
                                                    );
                                                })
                                            ) : (
                                ...
                                <>
                                    <div>
                                        ...
                                        <div className="relative mb-2">
                                            ...
                                            <input
                                                type="text"
                                                placeholder="Parça ara (İsim, PN, SN veya alt montaj grubu adı yazın)..."
                                        ...
                                        {removeSearchQuery && (
                                            <p className="text-xs text-blue-300 mb-2">
                                                📋 {filteredEngineComponents.length} parça bulundu
                                            </p>
                                        )}
                                        <select
                                            ...
                                            {filteredEngineComponents.length > 0 ? (
                                                filteredEngineComponents.map(comp => (
                                                    <option key={comp.id} value={comp.id}>
                                                        🔧 {comp.description} | PN: {comp.partNumber} | SN: {comp.serialNumber} | {comp.currentHours}h{comp.assemblyGroup ? ` | 📦 ${comp.assemblyGroup}` : ''}
                                                    </option>
                                                ))
                                            ) : (
                                        ...
                                        {swapState.removeId && (
                                            <p className="text-xs text-green-400 mt-1 flex items-center">
                                                ✓ Parça seçildi: <strong className="ml-1">
                                                    {engineComponents.find(c => c.id === parseInt(swapState.removeId))?.description}
                                                </strong>
                                            </p>
                                        )}
                                            ...
                                            {!swapState.installLater && (
                                                <>
                                                    <div className="relative mb-2">
                                                        ...
                                                        <input
                                                            type="text"
                                                            placeholder="Depoda parça ara (İsim, PN, SN yazın)..."
                                                    ...
                                                    {installSearchQuery && (
                                                        <p className="text-xs text-blue-300 mb-2">
                                                            📦 {filteredInventoryItems.length} parça bulundu
                                                        </p>
                                                    )}
                                                    <select
                                                    ...
                                                    {swapState.installId && (
                                                        <p className="text-xs text-blue-400 mt-1 flex items-center">
                                                            ✓ Parça seçildi: <strong className="ml-1">
                                                                {inventory?.find(i => i.id === parseInt(swapState.installId))?.description}
                                                            </strong>
                                                        </p>
                                                    )}
                        ...
                        <div className="p-12 text-center">
                            <p className="text-brand-light text-lg">
                                ⚠️ Parça değişimi yapmak için yetkiniz bulunmamaktadır.
                            </p>
                            <p className="text-brand-light text-sm mt-2">
                                Bu işlem için Administrator, Assembly Engineer veya Assembly Operator yetkisi gereklidir.
                            </p>
                        </div>
                    )}
                </div>
                                ...
                                <tr key={swap.id} className="border-b border-brand-border hover:bg-brand-dark transition-colors">
                                    ...
                                    <td className="p-3">
                                        <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                            ...
                                            {swap.swapType === 'BR Update' ? 'BR Güncelleme' : swap.swapType === 'Assembly' ? 'Parça Grubu' : 'Parça'}
                                        </span>
                                    </td>
                                    <td className="p-3 text-green-400 text-sm">
                                        {getDetailedComponentInfo(swap.componentInstalledId, swap.installedSerialNumber, swap.swapType === 'Assembly')}
                                    </td>
                                    <td className="p-3 text-red-400 text-sm">
                                        {getDetailedComponentInfo(swap.componentRemovedId, swap.removedSerialNumber, swap.swapType === 'Assembly')}
                                    </td>
            ...
            {editingSwap && (
                ...
                >
                    <form onSubmit={handleUpdateSwap} className="space-y-4">
                        ...
                        {/* If part was removed but not installed (installLater mode), allow installing now */}
                        {editingSwap.componentRemovedId && (!editingSwap.componentInstalledId || editingSwap.componentInstalledId === 0) && (
                            <div className="bg-green-500/10 border border-green-500/30 rounded p-3">
                                <p className="text-sm font-semibold text-green-400 mb-2">🔧 Yeni Parça Takma:</p>
                                <p className="text-xs text-brand-light mb-3">
                                    Bu kayıtta parça sadece sökülmüş, yeni parça takılmamış. Şimdi takabilirsiniz:
                                </p>
                                <label className="block text-sm font-medium text-brand-light mb-1">
                                    Depodan Takılacak Parça
                                </label>
                                <select
                                    value={(editingSwap as any).newPartId || ''}
                                    onChange={(e) => setEditingSwap({ ...editingSwap, newPartId: e.target.value } as any)}
                                    className="w-full bg-brand-dark border border-brand-border rounded-md p-2 text-white text-sm"
                                >
                                    <option value="">-- Parça seçin (opsiyonel) --</option>
                                    {inventory?.map(i => (
                                        <option key={i.id} value={i.id}>
                                            {i.description} (PN: {i.partNumber}, SN: {i.serialNumber})
                                        </option>
                                    ))}
                                </select>
                            </div>
                        ...
                        {(!editingSwap.componentRemovedId || editingSwap.componentInstalledId) && (
