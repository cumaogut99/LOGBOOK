...
router.post('/engines', async (req, res) => {
  try {
    ...
    const result = await dbRun(
      ...
      [model, serialNumber, status, totalHours || 0, totalCycles || 0, nextServiceDue, manufacturer, location, JSON.stringify(components || []), JSON.stringify(activityLog || [])]
    );
...
router.post('/tests', async (req, res) => {
  try {
    ...
    const result = await dbRun(
      'INSERT INTO tests (engineId, testType, brakeType, testCell, description, duration, testDate, documentId, userName) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [engineId, testType, brakeType, testCell, description, duration, testDate, documentId, userName]
    );
    res.json({ id: result.id, ...req.body });
  } catch (err) {
    res.status(500).json({ error: err.message });
