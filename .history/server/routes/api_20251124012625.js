...
// Check for duplicate serial numbers across the entire system
router.post('/check-duplicates', async (req, res) => {
  const { serialNumbers, excludeEngineId } = req.body;
  ...
  const duplicates = [];
  // Filter out generic serial numbers
  const targets = new Set(serialNumbers.filter(s => s && s !== '-' && s !== 'N/A').map(s => s.toString().trim()));

  if (targets.size === 0) return res.json({ duplicates: [] });

  try {
    // 1. Check Inventory
    const inventoryItems = await dbAll('SELECT serialNumber, location FROM inventory');
    for (const item of inventoryItems) {
      if (item.serialNumber && targets.has(item.serialNumber)) {
        duplicates.push({
          serialNumber: item.serialNumber,
          location: `Depo (Konum: ${item.location || '-'})`,
          type: 'inventory'
        });
      }
    }
    ...
    // 2. Check All Engines
    let enginesQuery = 'SELECT id, serialNumber, components FROM engines';
    const engines = await dbAll(enginesQuery);

    for (const engine of engines) {
      // Skip the engine we are currently editing
      if (excludeEngineId && engine.id === parseInt(excludeEngineId)) continue;
      ...
      const traverse = (comps) => {
        for (const comp of comps) {
          if (comp.serialNumber && targets.has(comp.serialNumber)) {
             duplicates.push({
               serialNumber: comp.serialNumber,
               location: `Motor: ${engine.serialNumber}`,
               type: 'engine',
               engineId: engine.id
             });
          }
    ...
    res.status(500).json({ error: 'Çakışma kontrolü yapılamadı' });
  }
});
...
router.post('/tests', async (req, res) => {
  try {
    const { engineId, testType, brakeType, operatorId, operatorName, testCell, description, duration, testDate, documentId, userName, assemblyGroupName, assemblyPartNumber, assemblySerialNumber } = req.body;
    const result = await dbRun(
      'INSERT INTO tests (engineId, testType, brakeType, operatorId, operatorName, testCell, description, duration, testDate, documentId, userName, assemblyGroupName, assemblyPartNumber, assemblySerialNumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [engineId, testType, brakeType, operatorId, operatorName, testCell, description, duration, testDate, documentId, userName, assemblyGroupName, assemblyPartNumber, assemblySerialNumber]
    );
    res.json({ id: result.id, ...req.body });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.put('/tests/:id', async (req, res) => {
  try {
    const { engineId, testType, brakeType, operatorId, operatorName, testCell, description, duration, testDate, documentId, userName, assemblyGroupName, assemblyPartNumber, assemblySerialNumber } = req.body;
    await dbRun(
      'UPDATE tests SET engineId = ?, testType = ?, brakeType = ?, operatorId = ?, operatorName = ?, testCell = ?, description = ?, duration = ?, testDate = ?, documentId = ?, userName = ?, assemblyGroupName = ?, assemblyPartNumber = ?, assemblySerialNumber = ? WHERE id = ?',
      [engineId, testType, brakeType, operatorId, operatorName, testCell, description, duration, testDate, documentId, userName, assemblyGroupName, assemblyPartNumber, assemblySerialNumber, req.params.id]
    );
...
router.put('/swaps/:id', async (req, res) => {
  try {
    ...
    const existing = await dbGet('SELECT * FROM swaps WHERE id = ?', [req.params.id]);
    if (!existing) {
    ...
    if (req.body.engineId !== undefined) updates.engineId = req.body.engineId;
    if (req.body.componentInstalledId !== undefined) updates.componentInstalledId = req.body.componentInstalledId || 0;
    if (req.body.componentRemovedId !== undefined) updates.componentRemovedId = req.body.componentRemovedId || 0;
    if (req.body.swapDate !== undefined) updates.swapDate = req.body.swapDate;
    if (req.body.swapType !== undefined) updates.swapType = req.body.swapType;
    if (req.body.assemblyGroup !== undefined) updates.assemblyGroup = req.body.assemblyGroup;
    if (req.body.documentId !== undefined) updates.documentId = req.body.documentId;
    if (req.body.userName !== undefined) updates.userName = req.body.userName;
    if (req.body.installedSerialNumber !== undefined) updates.installedSerialNumber = req.body.installedSerialNumber;
    if (req.body.removedSerialNumber !== undefined) updates.removedSerialNumber = req.body.removedSerialNumber;
    ...
    await dbRun(
      ...
      [updates.engineId, updates.componentInstalledId, updates.componentRemovedId, updates.swapDate, updates.swapType, updates.assemblyGroup, updates.documentId, updates.userName, updates.installedSerialNumber, updates.removedSerialNumber, req.params.id]
    );
    
    res.json({ id: req.params.id, ...updates });
  } catch (err) {
    console.error('PUT /swaps/:id error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/engines/:id/activities', async (req, res) => {
  try {
    ...
    swaps.forEach(swap => {
      activities.push({
        type: 'Swap',
        details: swap.swapType === 'Assembly' ? 'Parça Grubu Değişimi' : 'Parça Değişimi',
        date: swap.swapDate,
        id: swap.id,
        swapType: swap.swapType,
        assemblyGroup: swap.assemblyGroup,
        componentInstalledId: swap.componentInstalledId,
        componentRemovedId: swap.componentRemovedId,
        installedSerialNumber: swap.installedSerialNumber,
        removedSerialNumber: swap.removedSerialNumber,
        fullData: swap
      });
    });
