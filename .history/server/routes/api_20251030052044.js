...
// Upload document with file handling (supports both FormData and JSON)
router.post('/documents/upload', async (req, res) => {
  try {
    let fileName, fileData, fileType, fileSize, relatedType, relatedId, uploadedBy;
    
    // Check if it's FormData (multipart) or JSON
    if (req.body.file) {
      // Handle FormData - file is sent as base64 in body
