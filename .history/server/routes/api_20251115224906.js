...
router.post('/inventory', async (req, res) => {
  try {
    ...
    const result = await dbRun(
      'INSERT INTO inventory (partNumber, serialNumber, description, location, userName) VALUES (?, ?, ?, ?, ?)',
      [partNumber, serialNumber, description, location, userName]
    );
    res.json({ id: result.id, ...req.body });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.put('/inventory/:id', async (req, res) => {
  try {
    const { partNumber, serialNumber, description, location, userName } = req.body;
    await dbRun(
      'UPDATE inventory SET partNumber = ?, serialNumber = ?, description = ?, location = ?, userName = ? WHERE id = ?',
      [partNumber, serialNumber, description, location, userName, req.params.id]
    );
    res.json({ id: req.params.id, ...req.body });
  } catch (err) {
    res.status(500).json({ error: err.message });
