  ...
  try {
    ...
    for (const plan of periodicPlans) {
      ...
      if (isDue) {
        duePlans.push({
          ...plan,
          overdueHours: plan.totalHours - (plan.lastPerformedHours || 0) - plan.periodicIntervalHours
        });
      }
    }
    
    // Also get auto-generated one-time plans that are due
    const autoGeneratedPlans = await dbAll(
      `SELECT mp.*, e.totalHours, e.totalCycles, e.serialNumber as engineSerialNumber
       FROM maintenance_plans mp 
       JOIN engines e ON mp.engineId = e.id 
       WHERE mp.maintenanceType = 'one-time' 
       AND mp.status = 'Pending'
       AND mp.description LIKE '%Otomatik Oluşturuldu%'
       AND (
         (mp.dueHours IS NOT NULL AND e.totalHours >= mp.dueHours)
         OR
         (mp.dueCycles IS NOT NULL AND e.totalCycles >= mp.dueCycles)
       )`
    );
    
    // Add auto-generated plans to due plans
    for (const plan of autoGeneratedPlans) {
      duePlans.push({
        ...plan,
        overdueHours: plan.dueHours ? plan.totalHours - plan.dueHours : 0
      });
    }
...
router.patch('/maintenance-plans/:id/complete', async (req, res) => {
  try {
    ...
    // If this was generated from a periodic plan, update the parent and create next plan
    if (plan.description && plan.description.includes('Otomatik Oluşturuldu')) {
      // Find parent periodic plan
      const baseDescription = plan.description.replace(' (Otomatik Oluşturuldu)', '');
      const parentPlan = await dbGet(
        `SELECT * FROM maintenance_plans 
         WHERE engineId = ? 
         AND description = ? 
         AND maintenanceType = 'periodic'
         LIMIT 1`,
        [plan.engineId, baseDescription]
      );
      
      if (parentPlan) {
        let nextDueHours = null;
        if (parentPlan.periodicIntervalHours && engine) {
          nextDueHours = engine.totalHours + parentPlan.periodicIntervalHours;
        }
        
        // Update parent plan
        await dbRun(
          'UPDATE maintenance_plans SET lastPerformedHours = ?, nextDueHours = ? WHERE id = ?',
          [engine?.totalHours || 0, nextDueHours, parentPlan.id]
        );
        
        // Create next maintenance plan automatically
        if (nextDueHours && parentPlan.periodicIntervalHours) {
          // Calculate next scheduled date (estimate: assume 1 hour = 1 day for simplicity)
          const hoursUntilDue = nextDueHours - (engine?.totalHours || 0);
          const nextScheduledDate = new Date();
          nextScheduledDate.setDate(nextScheduledDate.getDate() + hoursUntilDue);
          
          // Check if there's already a pending plan for this periodic maintenance
          const existingNextPlan = await dbGet(
            `SELECT id FROM maintenance_plans 
             ...
             AND maintenanceType = 'one-time'
             LIMIT 1`,
            [plan.engineId, plan.description]
          );
          
          // Only create if there's no existing pending plan
          if (!existingNextPlan) {
            await dbRun(
              `INSERT INTO maintenance_plans 
               (engineId, planType, maintenanceType, description, scheduledDate, dueHours, createdBy, createdAt, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                plan.engineId,
                parentPlan.planType,
                'one-time',
                plan.description, // Keep the "(Otomatik Oluşturuldu)" suffix
