...
/**
 * Flattens component tree into a single array with assembly group info
 * @param components - Component tree to flatten
 * @returns Flat array of all components with assemblyGroup property
 */
export function flattenComponents(components: Component[]): Array<Component & { assemblyGroup?: string }> {
  const result: Array<Component & { assemblyGroup?: string }> = [];
  
  function traverse(comps: Component[], currentAssemblyGroup?: string) {
    comps.forEach(comp => {
      // If this component has children, it's an assembly group
      const isAssemblyGroup = comp.children && comp.children.length > 0;
      
      result.push({
        ...comp,
        assemblyGroup: isAssemblyGroup ? undefined : currentAssemblyGroup
      });
      
      if (isAssemblyGroup) {
        // Pass this component's description as the assembly group for its children
        traverse(comp.children, comp.description);
      }
    });
  }
